		...
		(error, results) => {
		  ...
		  connection.query(
			...
			(error, results) => {
			  ...
			  //席を更新
			  connection.query(
				'UPDATE seats SET user_id = ? WHERE seat_id = ?',
				[req.body.userId, req.body.seatId],
				(error, results) => {
				  if (error) {
					return connection.rollback(() => {
					  res.status(500).json({ error: 'クエリに失敗しました' });
  ...
  router.put('/vacate', (req, res) => {
	...
	connection.query(
	  ...
	  (error, results) => {
		if (error) {
			console.log(error);
		  return res.status(500).json({ error: 'クエリに失敗しました(着席の確認)' });
		}
		...
		// 席の開放
		connection.query(
			'UPDATE seats SET user_id = NULL WHERE seat_id = ?',
			[req.body.seatId],
			(error, results) => {
			  if (error) {
				  res.status(500).json({ error: 'クエリに失敗しました' });
